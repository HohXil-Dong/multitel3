c:        program readray
        subroutine readray(gcarc,para,geom,time,tstar,ref_co,takeoff,
     &                    nray)
...
c            write(*,*) inputfile, rayindex
            ...
            read (i+20, "(i6)") raynum
            if (raynum .gt. max_ray) then
                write(*,*) 'Error: raynum exceeds max_ray for phase ',i
                stop 2
            endif
            nray(i) = raynum
c            write (*,*) raynum
     ...
     &              time(j,i),tstar(j,i),ref_co(j,i),takeoff(j,i)
            end do
            close (i+20)
        end do
